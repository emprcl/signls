...
func (k KeyMode) Display() string {
	return k.node.(core.Emitter).Note().NoteBehavior.Name()
}
...
func (k KeyMode) Set(value int) {
	...
	k.node.(core.Emitter).Note().NoteBehavior = k.modes[value]
}

func (k KeyMode) keyModeIndex() int {
	for i := 0; i < len(k.modes); i++ {
		if k.node.(core.Emitter).Note().NoteBehavior == k.modes[i] {
			return i
		}
	}
	return 0
