...
func BenchmarkGrid(b *testing.B) {
	...
	for _, v := range benchmarks {
		b.Run(fmt.Sprintf("grid_size_%dx%d", v.size, v.size), func(b *testing.B) {
			...
			grid.AddNode(node.NewRelayEmitter(midi, common.DOWN), 11, 7)
			grid.AddNode(node.NewRelayEmitter(midi, common.LEFT), 11, 11)
			grid.AddNode(node.NewRelayEmitter(midi, common.UP), 7, 11)
			grid.AddNode(node.NewBangEmitter(midi, common.RIGHT, true), 7, 2)
			grid.AddNode(node.NewRelayEmitter(midi, common.LEFT), 12, 2)
			grid.AddNode(node.NewBangEmitter(midi, common.RIGHT, true), 7, 3)
			grid.AddNode(node.NewRelayEmitter(midi, common.LEFT), 9, 3)
			for i := 0; i < b.N; i++ {
				grid.Update()
			}
		})
